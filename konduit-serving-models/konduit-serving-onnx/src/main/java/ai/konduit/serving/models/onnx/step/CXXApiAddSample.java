...
public class CXXApiAddSample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        long input_tensor_size = 1;  // simplify ... using known dim values to calculate size
        // use OrtGetTensorShapeElementCount() to get official size!

        FloatPointer input_tensor_values = new FloatPointer(input_tensor_size);
        FloatPointer input_tensor_values2 = new FloatPointer(input_tensor_size);
        ...
        idx.put(0,1.0f);
        FloatIndexer idx2 = FloatIndexer.create(input_tensor_values2);
        idx2.put(0,1.0f);

        // create input tensor object from data values
        MemoryInfo memory_info = MemoryInfo.CreateCpu(OrtArenaAllocator, OrtMemTypeDefault);
        Value values = new Value(2);
        values.position(0).put(Value.CreateTensorFloat(memory_info.asOrtMemoryInfo(), input_tensor_values, input_tensor_size, input_node_dims, 2));
        values.position(1).put(Value.CreateTensorFloat(memory_info.asOrtMemoryInfo(), input_tensor_values, input_tensor_size, input_node_dims, 2));
        values.position(0);
        Value input_tensor = Value.CreateTensorFloat(memory_info.asOrtMemoryInfo(), input_tensor_values, input_tensor_size, input_node_dims, 2);
