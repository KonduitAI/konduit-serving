...
public class ClassificationMetrics implements MetricsRenderer {
    ...
    public ClassificationMetrics(ClassificationMetricsConfig classificationMetricsConfig) {
        this(classificationMetricsConfig, Collections.singletonList(new ImmutableTag("machinelearning", "classification")));
    }
    ...
    public void bindTo(MeterRegistry meterRegistry) {
        List<String> labels = classificationMetricsConfig.getClassificationLabels();
        for (String label : labels) {
            classCounterIncrement.add(Counter.builder(label)
                    .tags(tags)
                    .description("Classification counts seen so far for label " + label)
