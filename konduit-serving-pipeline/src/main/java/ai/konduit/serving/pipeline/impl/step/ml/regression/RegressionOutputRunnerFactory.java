...
public class RegressionOutputRunnerFactory implements PipelineStepRunnerFactory {

    @Override
    public boolean canRun(PipelineStep step) {
        return step instanceof RegressionOutputStep;
    }

    @Override
    public PipelineStepRunner create(PipelineStep step) {
        Preconditions.checkState(canRun(step), "Unable to run step: %s", step);
        return new RegressionOutputRunner((RegressionOutputStep) step);
    }
}
