...
public class ClassifierOutputRunnerFactory implements PipelineStepRunnerFactory {

    @Override
    public boolean canRun(PipelineStep step) {
        return step instanceof ClassifierOutputStep;
    }

    @Override
    public PipelineStepRunner create(PipelineStep step) {
        Preconditions.checkState(canRun(step), "Unable to run step: %s", step);
        return new ClassifierOutputRunner((ClassifierOutputStep) step);
    }
}
