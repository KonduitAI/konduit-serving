...
public class SSDToBoundingBoxRunnerFactory implements PipelineStepRunnerFactory {

    @Override
    public boolean canRun(PipelineStep step) {
        return step instanceof SSDToBoundingBoxStep;
    }

    @Override
    public PipelineStepRunner create(PipelineStep step) {
        Preconditions.checkState(canRun(step), "Unable to run step: %s", step);
        return new SSDToBoundingBoxRunner((SSDToBoundingBoxStep) step);
    }
}
